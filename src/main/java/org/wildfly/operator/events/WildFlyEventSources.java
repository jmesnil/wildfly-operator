...
public class WildFlyEventSources extends AbstractEventSource implements Watcher<StatefulSet> {

    private static final Logger log = LoggerFactory.getLogger(WildFlyEventSources.class);

    private final KubernetesClient client;
    private final OperatorConfig config;

    public WildFlyEventSources(KubernetesClient client, OperatorConfig config) {
        this.client = client;
        this.config = config;
    ...
    private void registerWatch(KubernetesClient client) {
        log.info("Watch statefulsets resources matching the label: \"app.kubernetes.io/managed-by: {} \"", config.getManagedByLabel());
        client
                .apps()
                .statefulSets()
                .inAnyNamespace()
                .withLabel("app.kubernetes.io/managed-by", config.getManagedByLabel())
                .watch(this);
    }
